...
**Acceptance Criteria**:
- [x] 길이가 1 미만이면 에러를 반환한다 (REQ-3)
- [x] 길이가 1024 초과이면 에러를 반환한다 (REQ-3)
- [x] 모든 문자셋이 비활성화되면 에러를 반환한다 (REQ-2, RISK-2)
- [x] 에러 메시지가 명확하고 사용자 친화적이다
- [x] count가 1 미만이면 에러를 반환한다 (REQ-5)

**Self-review checklist**:
- [x] 경계값(1, 1024)에서 정상 동작하는가
- [x] 에러 메시지가 TRD 섹션 5.2의 정의와 일치하는가
- [x] 숫자가 아닌 값이 전달될 경우 처리되는가
...
**Acceptance Criteria**:
- [x] 기본 옵션으로 16자 패스워드가 생성되는지 테스트한다 (FEAT-1)
- [x] 지정된 길이로 패스워드가 생성되는지 테스트한다 (FEAT-2)
- [x] count 옵션으로 다수의 패스워드가 생성되는지 테스트한다 (FEAT-4)
- [x] 각 문자셋 필터링이 올바르게 동작하는지 테스트한다 (FEAT-3)
- [x] 모든 문자셋 비활성화 시 에러가 발생하는지 테스트한다 (REQ-2, RISK-2)
- [x] 길이 경계값(1, 1024, 0, 1025)에 대한 테스트가 있다 (REQ-3)
- [x] 생성된 패스워드가 요청된 문자셋만 포함하는지 검증한다
- [x] `npm test` 실행 시 모든 테스트가 통과한다

**Self-review checklist**:
- [x] 엣지 케이스(길이 1, 최대 길이, 단일 문자셋만 활성화 등)가 커버되는가
- [x] 테스트가 결정적(deterministic)인가 (랜덤이지만 속성 기반 검증)
- [x] `generator.ts`의 커버리지가 90% 이상인가
...
**Acceptance Criteria**:
- [x] 기본 실행(`randpw`)에서 16자 패스워드가 출력되는지 테스트한다
- [x] `-l 32` 옵션으로 32자 패스워드가 출력되는지 테스트한다
- [x] `-c 5` 옵션으로 5개 패스워드가 출력되는지 테스트한다
- [x] `--no-symbols` 등 문자셋 필터링 옵션이 동작하는지 테스트한다
- [x] `--version` 옵션이 버전을 출력하는지 테스트한다
- [x] `--help` 옵션이 도움말을 출력하는지 테스트한다
- [x] 잘못된 옵션 시 종료코드 1이 반환되는지 테스트한다
- [x] 모든 문자셋 제외 시 에러 메시지가 출력되는지 테스트한다
- [x] `npm test` 실행 시 모든 테스트가 통과한다

**Self-review checklist**:
- [x] CLI 테스트가 실제 프로세스 실행(child_process) 방식으로 구현되어 있는가
- [x] stdout과 stderr를 분리하여 검증하는가
- [x] 종료코드를 검증하는가
...
| M0 | TASK-002 | 의존성 설치 및 빌드 도구 구성 | NFR-7 | `completed` |
| M0 | TASK-003 | 코드 품질 도구 구성 (ESLint + Prettier) | RISK-1 | `completed` |
| M0 | TASK-004 | 테스트 프레임워크 구성 및 샘플 테스트 | NFR-5 | `completed` |
| M0 | TASK-005 | 통합 스크립트 등록 및 환경 검증 | - | `completed` |
| M1 | TASK-006 | 문자셋 상수 및 타입 정의 | FEAT-1, FEAT-3 | `completed` |
| M1 | TASK-007 | 패스워드 생성 핵심 함수 구현 | FEAT-1, REQ-1, RISK-1 | `completed` |
| M1 | TASK-008 | 입력 검증 로직 구현 | REQ-2, REQ-3, REQ-5, RISK-2 | `completed` |
| M1 | TASK-009 | generator 단위 테스트 작성 | FEAT-1~4, REQ-1~4 | `completed` |
| M2 | TASK-010 | CLI 엔트리포인트 구현 | FEAT-2~5 | `completed` |
| M2 | TASK-011 | CLI 통합 테스트 작성 | FEAT-1~5 | `completed` |
| M3 | TASK-012 | 빌드 검증 및 패키지 구성 완료 | NFR-7 | `completed` |
| M3 | TASK-013 | README.md 작성 | - | `completed` |
| M3 | TASK-014 | npm publish 준비 및 최종 검증 | NFR-2, NFR-3 | `completed` |
